...
install_package()
{
	...
	if (( EXPORT==2 )); then
		msg $(eval_gettext 'Exporting ${pkgbase} to ${CACHEDIR} repository')
		launch_with_su cp -vf "$YPKGDEST/"* "$CACHEDIR/" 
...
get_pkgbuild ()
{
	local i pkgs repo pkg pkgbase pkgver arch pkgbuild_tmp deps cwd=$(pwd)
	pkgs=("$@")
	loadlibrary aur abs
